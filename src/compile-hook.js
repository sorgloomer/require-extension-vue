...
const compileHook = (code, filename) => {
  log.info(`[require-extension-vue info] permanent cache is ${isPermanentCacheEnabled() ? 'enabled' : 'not enabled'}`);
  ...
  if (cachedFile) {
    ...
    log.debug(`[require-extension-vue debug] cached file: ${JSON.stringify(cachedFile, null, 2)}`);
    return cachedFile;
  }
  if (compiling) {
